...
ModuleHeader MOD_HEADER(nopost)
  = {
	"nopost",
	"v1.1",
...
DLLFUNC int nopost_config_test(ConfigFile *, ConfigEntry *, int, int *);
DLLFUNC int nopost_config_run(ConfigFile *, ConfigEntry *, int);
...
DLLFUNC int MOD_LOAD(nopost)(int module_load)
{
	return MOD_SUCCESS;
}

DLLFUNC int MOD_UNLOAD(nopost)(int module_unload)
{
	free_config();
	return MOD_SUCCESS;
}
...
DLLFUNC int nopost_config_test(ConfigFile *cf, ConfigEntry *ce, int type, int *errs)
{
int errors = 0;
ConfigEntry *cep;
